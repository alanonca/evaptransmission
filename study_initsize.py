...
mode_to_test = 'coughing' # enter 'speaking','coughing', or 'breathing'
...
for condT in range(len(T_range)): # Study the effect of varying temperature with fixed RH at 50% 
    ...
    if mode_to_test == 'speaking':
        [sizeclass,numcon,t_settle] = module_initsize.speaking(T,RH)
    elif mode_to_test == 'coughing':
        [sizeclass,numcon,t_settle] = module_initsize.coughing(T,RH)
    elif mode_to_test == 'breathing':
        [sizeclass,numcon,t_settle] = module_initsize.breathing(T,RH)
    c = colors[condT]
...
for condRH in range(len(RH_range)): # Study the effect of varying RH with fixed T at 25 C
    ...
    popt, pcov = curve_fit(module_initsize.gaussian, t_settle, numcon,p0)
    rsq = module_initsize.gfit(t_settle,numcon,popt)
    print(rsq)
    tspan = np.linspace(0,np.amax(t_settle),50)
    numfitcurve = module_initsize.gaussian(tspan,popt[0],popt[1],popt[2])
...
plt.axis([0, 0.005, 0, 1.2*np.amax(numcon)])
